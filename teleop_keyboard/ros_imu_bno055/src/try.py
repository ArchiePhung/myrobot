...
# def bytes_to_int(data): # bytes (multiple) to int (256)
# 	val = 0
# 	le = len(data)
# 	print ("le: ", le)
# 	val = struct.unpack('<H', data)[0]

# 	if (val > math.pow(256, 2)/2 ):
# 		val = val - math.pow(256, 2)
...
def build_write_command( address, data):

    try:
        data_length = int_byte_size(data) # Returns the number of bytes used by int variable
    except:
...
N2 = b'\x00\x01\x02\xF1'

# cmd = build_write_command(OPR_MODE, CONFIGMODE)
# print ("cmd: ", cmd)

print ("AA: ", bytes_to_int(N2) )