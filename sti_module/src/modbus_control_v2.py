        ...
        while not self.shutdown_flag.is_set():
          ...
          # s4x write
            if id_modbus_recived == True :
                id_modbus_recived = False
                rospy.logwarn("send Modbus -> Main ")
...
class Connect_ros(threading.Thread):
    def __init__(self, threadID):
        ...
        self.mission_status = rospy.Publisher('mission_status',Mission_status , queue_size=50)
        # rospy.Subscriber("modbus_4x_write", Modbus_4x_write, self.call_modbus)
        rospy.Subscriber("mission_control", Mission_control, self.call_mission)
    ...
    def call_mission(self, data):
		global data_4x_write ,id_modbus_recived
		if self.is_mission == False :
			self.is_mission = True
		self.mission_control = data

		data_4x_write.Led_FC = self.mission_control.lighting
		data_4x_write.COI_FC = self.mission_control.speaker
		data_4x_write.Cmd_move  = self.mission_control.reset_error # reset ALL
		for i in range(3):
			id_modbus_recived = True
		# print("hihi")
    ...
    def pub_missonStatus(self, stt , pin , err , bt_stt):
        msg = Mission_status()
        msg.mission_status = stt 
        msg.battery = pin 
        msg.error = err 
        msg.lifttable_robot_status = bt_stt
        self.mission_status.publish(msg)
    ...
    def run(self): 
        ...
        while not self.shutdown_flag.is_set():
            ...
            self.SS_BT1_1 = self.chuyendoi(data_4x_read.Sie_in,1) 
            self.SS_BT1_2 = self.chuyendoi(data_4x_read.Sie_in,2) 
            self.SS_BT2_1 = self.chuyendoi(data_4x_read.Sie_in,4) 
            self.SS_BT2_2 = self.chuyendoi(data_4x_read.Sie_in,3) 
            self.BT_EN    = self.chuyendoi(data_4x_read.Sie_in,7) 
            self.SS_ngoai = self.chuyendoi(data_4x_read.BT_SIE,8) 
          # reset
            if self.mission_control.enable == 0 :
				rospy.logwarn("reset")
				data_4x_write.BT_DC_1 = 0 # BT agv stop
				data_4x_write.BT_SIE_OUT = int(self.bt_stop) # BT stop 
				for i in range(3):
					id_modbus_recived = True  # gui modbus
				process = 1
				step = 1 

          # get mission_id
            if process == 1 : 
                self.pub_missonStatus(-1, data_4x_read.Battery_value, 0, self.SS_ngoai)
                rospy.loginfo("process: %s",process)
                if self.is_mission == True and self.mission_control.enable == 1:
                    self.is_mission = False
                    if self.mission_control.mission == 1 :      
                        mission_id = 2 # hang AGV -> BT 
                        process = 2
                    if self.mission_control.mission == 2 :      
                        mission_id = 3 # hang BT  -> AGV
                        process = 2

                    if self.mission_control.lifttable == 1 :    
                        mission_id = 4 # nang
                        process = 2
                    if self.mission_control.lifttable == 2 :    
                        mission_id = 5 # ha
                        process = 2

                    if self.mission_control.conveyor == 1 :     
                        mission_id = 6 # quay trai
                        process = 2
                    if self.mission_control.conveyor == 2 :     
                        mission_id = 7 # quay phai
                        process = 2
          ...
          # working
            if process == 2 : 
                # rospy.loginfo("process: %s",process)
                if mission_id == 2: # hang AGV -> BT
					if step == 1 : 
						if self.BT_EN == True : # bt KO hoat dong
							self.pub_missonStatus(1, data_4x_read.Battery_value, 5, self.SS_ngoai) 
						else : step = 2

					if step == 2 :
						if self.SS_ngoai == True : # ko co hang ten AGV
							self.pub_missonStatus(1, data_4x_read.Battery_value, 1, self.SS_ngoai)
						else : step = 3

					if step == 3 :
						if self.SS_BT1_2 == False : # da co hang tren BT
							self.pub_missonStatus(1, data_4x_read.Battery_value, 4, self.SS_ngoai)
						else : step = 4

					if step == 4 :
						self.pub_missonStatus(1, data_4x_read.Battery_value, 0 , self.SS_ngoai)
						data_4x_write.BT_SIE_OUT = int(self.bt1_in) # dieu khien bang tai q.Trai
						data_4x_write.BT_DC_1 = 2 # BT agv quay trai
						for i in range(3):
							id_modbus_recived = True  # gui modbus
						step = 5

					if step == 5 :   
						if self.SS_BT1_2 == True : # cho chuyen hang
							pass
                ...
                if mission_id == 3: # hang BT  -> AGV
                    ...
                    if step == 2 :
                        if self.SS_BT1_2 == True : # ko co hang tren BT
                            self.pub_missonStatus(1, data_4x_read.Battery_value, 2, self.SS_ngoai)
                        else : step = 3
                    
                    if step == 3 :
                        if self.SS_ngoai == False : # da co hang tren AGV
                            self.pub_missonStatus(1, data_4x_read.Battery_value, 3, self.SS_ngoai)
                        else : step = 4

                    if step == 4 :
                        self.pub_missonStatus(1, data_4x_read.Battery_value, 0 , self.SS_ngoai)
                        data_4x_write.BT_SIE_OUT = int(self.bt1_out) # dieu khien bang tai q.Phai
                        data_4x_write.BT_DC_1 = 1 # BT agv quay phai
                        for i in range(3):
							id_modbus_recived = True  # gui modbus
                        step = 5

                    if step == 5 :   
                        if self.SS_ngoai == True : # cho chuyen hang
                            pass
                ...
                if mission_id == 4: # nang 
                    if step == 1 :
                        data_4x_write.BN_FC = 1 
                        for i in range(3):
							id_modbus_recived = True # gui modbus
                        step = 2
                    if step == 2 : 
                        if  data_0x.BN_SS_H == True : process = 3

                if mission_id == 5: # ha
                    if step == 1 :
                        data_4x_write.BN_FC = 2
                        for i in range(3):
							id_modbus_recived = True # gui modbus
                        step = 2 
                    if step == 2 :
                        if  data_0x.BN_SS_L == True : process = 3
                
          # ok , wait reset
            if process == 3 : 
                self.pub_missonStatus(2, data_4x_read.Battery_value, data_0x.Err_all, self.SS_ngoai)
                if self.mission_control.enable == 0 :
                    rospy.logwarn("reset")
                    process = 1
                    step = 1 
