...
class test:
	def __init__(self):
		...
		self.pub6 = rospy.Publisher('/POWER_request',POWER_request , queue_size=50)
		self.pub7 = rospy.Publisher('/HC_request',HC_request , queue_size=50)
		self.pub8 = rospy.Publisher('/lift_control',Lift_control , queue_size=50)
	...
	def test_power(self): # mach MAIN8.2
		p = POWER_request()
		p.charge = True
		p.sound_on = True
		p.sound_type = 4
		...
		p.led_button1 = False
		p.led_button2 = False
		# float32 a_coefficient
		# float32 b_coefficient
		for i in range(2):
			self.pub6.publish(p)
			print "power"
			rospy.sleep(0.1)

	def test_HC(self): # mach HC8.2
		p = HC_request()
		p.RBG1 = 3
		p.RBG2 = 200
