...
class StiAutomaticParking():
	def __init__(self):
		...
		rospy.init_node('parking_lintu_v11', anonymous=True)
	...
	def run1_lineMag_checkke(self, vel ):
		...
		if self.step_runlintu == 0 :
			...
			if self.job == 2 : 	
				self.dem_time_checkke = 0 
				self.dem_time_checkke_yes = 0
				self.t3_ht = self.t3_tr = rospy.get_time()
				self.pub_cmd_vel.publish(Twist())
		...
		if self.step_runlintu == 1 : # check ke 
			...
			if self.dem_time_checkke < 20 : 
				rospy.logwarn("check_ke time= {}".format(self.t3_ht - self.t3_tr))
				rospy.logwarn("dem_time_checkke= {}".format(self.dem_time_checkke))
				if self.t3_ht - self.t3_tr > 0.2  :
					self.t3_tr = self.t3_ht = rospy.get_time()
					self.dem_time_checkke = self.dem_time_checkke+ 1
					if self.zone_robot.zone_ahead == 2 : 
	...
	def raa(self):
		...
		while not rospy.is_shutdown() :
			# if self.is_zone_robot == False :
			# 	self.is_zone_robot = True
			# else :
