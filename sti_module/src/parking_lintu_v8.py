...
class StiAutomaticParking(threading.Thread):
    ...
    def run(self):
        ...
        while (not self.shutdown_flag.is_set()) or (not rospy.is_shutdown()) :
          # reset
          ...
          #B30: check line tu     
            if buoc == 30 : 
                ...
                # if self.rb_z > (self.kc_tag_robot_setup + 0.05) : # robot dung xa dau linetu 
                #     self.s_runros = self.rb_z - self.kc_tag_robot_setup
                #     self.step_runros = 0
                #     buoc = 31

                # elif self.rb_z < (self.kc_tag_robot_setup - 0.05) : # robot dung qua gan
                #     self.s_runros = self.rb_z - self.kc_tag_robot_setup 
                #     self.step_runros = 0
                #     buoc = 31
            ...
            if buoc == 33 :
                ...
                if(goc_taget > 0):
                    ...
                    if math.fabs(self.odom_g - goc_taget) > 0.04 :
                        ...
                        if twist.angular.z > 0.25 : twist.angular.z = 0.25
                        if twist.angular.z < -0.25 : twist.angular.z = -0.25
                        self.pub_cmdVel(twist,self.rate_cmdvel,rospy.get_time())
                    else:
          ...
          #B9: tinh tien di vao  , NOTE : check yes/no Tag      
            if buoc == 9 :
                ...
                if self.run1_lineMag(self.vel_line) == True : 
...
class ReadApriltag(threading.Thread):
    ...
    def fnShutDown(self):
        rospy.loginfo("Shutting down thread 2")
        # self.pub_cmd_vel.publish(Twist())
    ...
    def cbGetMarkerOdom(self, markers_odom_msg): # 5Hz
        global is_marker_pose_received , tag_g , is_marker_pose_received2 , find_id , idTag_target

        if is_marker_pose_received2 == False:
            is_marker_pose_received2 = True

        sl_tag = len(markers_odom_msg.detections)
        print("sl_tag= {}".format(sl_tag))
        if sl_tag == 0 : 
            find_id = 0 
            tag_g = .0 
        else :
            for sl_tag in range(sl_tag):            
                ...
                if id ==  idTag_target : 
                    ...
                    find_id = id
                    print("id == idTag_target= {}".format(find_id))
    ...
    def cbGetMarkerOdom_old(self, markers_odom_msg): # 5Hz
        global is_marker_pose_received , tag_g , is_marker_pose_received2 , find_id , idTag_target
        ...
        if sl_tag == 0 : 
            ...
            for sl_tag in range(sl_tag):            
                ...
                if find_id == idTag_target : 
                    print("find_id == idTag_target= {}".format(find_id))
                    marker_odom = markers_odom_msg.detections[sl_tag-1]
