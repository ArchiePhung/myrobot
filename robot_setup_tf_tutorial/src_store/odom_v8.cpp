...
{   
    if(!isnan(odom_hihi.pose.pose.position.x)) // kiem tra data khac "nan"
    {
        ...
        px_ht_t265 = odom_hihi.pose.pose.position.x ;
        py_ht_t265 = d + odom_hihi.pose.pose.position.y ; // t265 nam ben TRAI robot
        th_ht_t265 = yaw_rad ;
...
int main(int argc, char** argv){
  ...
  while(n.ok()){
    ...
    //set the position
    odom_robot.pose.pose.position.x = py_robot;
    odom_robot.pose.pose.position.y = px_robot;
    ...
    //set the velocity
    odom_robot.twist.twist.linear.x = vy_robot;
    odom_robot.twist.twist.linear.y = vx_robot;
    ...
    // phai de khac : odom , base_footprint , k la Rviz nhay 
    odom_trans2.header.frame_id = "odom_rb";
    odom_trans2.child_frame_id = "base_robot";

    odom_trans2.transform.translation.x = py_robot;
    odom_trans2.transform.translation.y = px_robot;
